...
sleep 15
curl -X POST --data-binary @$currentDir/SchedulePipelineFunctionApp.zip -H "Authorization: Bearer $access_token_app" "https://TimerStartPipelineFunction-$prefix.scm.azurewebsites.net/api/zipdeploy"
...
sleep 15
curl -X POST --data-binary @$currentDir/AdxIngestFunctionApp.zip -H "Authorization: Bearer $access_token_app" "https://AdxIngestFunction-$prefix.scm.azurewebsites.net/api/zipdeploy"
